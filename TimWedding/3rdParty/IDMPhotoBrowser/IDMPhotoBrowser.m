...
- (void)viewDidLoad {
    ...
    // View
	self.view.backgroundColor = [UIColor colorWithWhite:(_useWhiteBackgroundColor ? 1 : 0) alpha:0.0];
    ...
    if(!_doneButtonImage) {
        ...
        [_doneButton setTitle:IDMPhotoBrowserLocalizedStrings(@"Close") forState:UIControlStateNormal];
        [_doneButton.titleLabel setFont:[UIFont fontWithName:defaultFont size:12]];
        [_doneButton setBackgroundColor:[UIColor colorWithWhite:0.1 alpha:0.0]];
    ...
    _counterLabel.font = [UIFont fontWithName:defaultFont size:14];
...
//	}
    ...
    if ([self numberOfPhotos] > 0) {
        _counterLabel.text = [NSString stringWithFormat:@"%lu/%lu", _currentPageIndex+1, (unsigned long)[self numberOfPhotos]];
    } else {
        _counterLabel.text = nil;
    }
