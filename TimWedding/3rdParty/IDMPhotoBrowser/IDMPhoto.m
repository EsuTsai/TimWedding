...
- (void)loadUnderlyingImageAndNotify {
    ...
    if (self.underlyingImage) {
        ...
        if (_photoPath) {
            ...
            //修正AFNetworking變成disk cache(原本是memory cache)
            NSURLRequest *urlRequest = [NSURLRequest requestWithURL:_photoURL cachePolicy:NSURLRequestReturnCacheDataElseLoad timeoutInterval:0.0f];
            UIImage *image = [[SDImageCache sharedImageCache] imageFromMemoryCacheForKey:[_photoURL absoluteString]];

//            UIImage *image = [[UIImageView sharedImageCache] cachedImageForRequest:urlRequest];
            if (image != nil) {
                if(SCREEN_BOUNDS.size.width == 414 && image.size.width <400){
                    image = [self imageByScalingAndCroppingForSize:CGSizeMake(image.size.width*3, image.size.height*3) image:image];
                }else if (SCREEN_BOUNDS.size.width == 375 && image.size.width < 350){
                    image = [self imageByScalingAndCroppingForSize:CGSizeMake(image.size.width*2, image.size.height*2) image:image];
                }else if(SCREEN_BOUNDS.size.width == 320 && image.size.width < 300){
                    image = [self imageByScalingAndCroppingForSize:CGSizeMake(image.size.width*2, image.size.height*2) image:image];
                }
                self.underlyingImage = image;
